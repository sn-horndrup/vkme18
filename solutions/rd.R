...
pacman::p_load(tidyverse,rdd)

#indlæs data
eh<-read_csv("data/13_eh.csv")
...
olsrd_tory<-lm(ln.net~margin+I(margin>0),data=filter(eh,party=="tory" & margin >= -.15 & margin <= .15))
summary(olsrd_tory)

olsrd_labour<-lm(ln.net~margin+I(margin>0),data=filter(eh,party=="labour" & margin >= -.15 & margin <= .15))
